...
\begin{enumerate}
    \item HIBAG can be used by researchers with published parameter estimates (\url{http://www.biostat.washington.edu/~bsweir/HIBAG/} \footnote{mirror site: \url{http://corearray.sourceforge.net/mirror/HIBAG/}}) instead of requiring access to large training sample datasets.
...
%     ---- Release a HIBAG model with no confidential information ----
\subsection{Release a HIBAG model with no confidential information}
...
# remove unused SNPs from the model and sample IDs
