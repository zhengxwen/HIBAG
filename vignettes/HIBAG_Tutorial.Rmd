...
# violin plot
library(ggplot2)
dat <- data.frame( matching=c(model$matching, pred$value$matching),
    dataset=c(rep("training", length(model$matching)), rep("test", length(pred$value$matching))) )
ggplot(dat, aes(x=dataset, y=matching)) +  geom_violin() +
    geom_jitter(position=position_jitter(0.2)) + ylab("matching distribution")
```
