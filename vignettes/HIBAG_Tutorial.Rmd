...
```{r}
# prepare data
fn <- system.file("extdata", "case_control.txt.xz", package="HIBAG")
# or fn <- "/your_folder/file.txt"

dat <- read.table(fn, header=TRUE, stringsAsFactors=FALSE)
head(dat)

hla <- hlaAllele(dat$sample.id, H1=dat$A, H2=dat$A.1, locus="A", assembly="hg19")
```
...
```{r}
hlaAssocTest(hla, disease ~ h, data=dat)  # 95% confidence interval (h.25%, h.75%)
...
# association tests
hlaAssocTest(aa, disease ~ h, data=dat, model="dominant")

hlaAssocTest(aa, disease ~ h, data=dat, model="recessive")
```
