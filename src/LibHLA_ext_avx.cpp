	...
	const THaplotype &H1, const THaplotype &H2)
{
	...
	if (Length <= GENO_HALF_NBIT)
	{
		...
		__m256d M = _mm256_andnot_pd(_mm256_castsi256_pd(S1),
			_mm256_castsi256_pd(S2));  // missing value, 1 is missing
		__m256d M2 = { M[0], M[1], M[0], M[1] };
		__m256i MASK = _mm256_castpd_si256(
			_mm256_andnot_pd(M2, _mm256_castsi256_pd(m1 | m2)));
	#else
